...
directory="/usr/share/icons/Papirus-Dark/16x16/"
...
selection=$(echo -ne "$final_rofi" | rofi -markup-rows -theme ~/.config/rofi/noti-center.rasi -dmenu -eh 3 -a "$active_list" -u "$urgent_list" -sep '\x0f' -p "Notification Center" -no-fixed-num-lines -lines 8 -i -no-config)
